...
let s:CMDFORMAT_CLONE = 'git -c credential.helper= clone --no-tags --single-branch --depth 1 %s https://github.com/%s/%s.git'
...
function! pkgsync#common#get_pluginname(p) abort
  if type('') == type(a:p)
    return a:p
  elseif type({}) == type(a:p)
    return get(a:p, 'name', '')
  else
    return ''
  endif
...
function! pkgsync#common#get_pluginbranch(p) abort
  if type({}) == type(a:p)
    return get(a:p, 'branch', '')
  else
    return ''
  endif
endfunction

function! pkgsync#common#find_pluginname(plugins, name) abort
  for i in range(0, len(a:plugins) - 1)
    if type('') == type(a:plugins[i])
      if a:name == a:plugins[i]
        return i
      endif
    elseif type({}) == type(a:plugins[i])
      if a:name == get(a:plugins[i], 'name')
        return i
      endif
    endif
  endfor
...
function! pkgsync#common#write_config(j) abort
  ...
  for start_or_opt in start_or_opts
    ...
    for user_name in user_names
      let plugins = sort(a:j['plugins'][start_or_opt][user_name])
      let lines += [printf('%s"%s": [', "\t\t\t", user_name)]
      for i in range(0, len(plugins) - 1)
        let plugin = plugins[i]
        let is_last = i == (len(plugins) - 1)
        if type(plugin) == type('')
          let lines += [printf('%s"%s"%s', "\t\t\t\t", plugin, !is_last ? ',' : '')]
        elseif type(plugin) == type({})
          let s = ''
          for key in ['name', 'branch']
            if has_key(plugin, key)
              let s = s .. printf('%s"%s": "%s"', (empty(s) ? '' : ', '), key, plugin[key])
            endif
          endfor
          let lines += [printf('%s{ %s }%s', "\t\t\t\t", s, !is_last ? ',' : '')]
        endif
      endfor
...
function! pkgsync#common#make_params(pack_dir, start_d, opt_d) abort
  ...
  for d in [a:start_d, a:opt_d]
    ...
    for user_name in keys(d)
      ...
      for plugin in d[user_name]
        let plugin_dir = pack_dir .. '/' .. pkgsync#common#get_pluginname(plugin)
        if isdirectory(plugin_dir)
          let params += [{
            \   'name': printf('%s/%s', user_name, pkgsync#common#get_pluginname(plugin)),
          ...
          let branch = pkgsync#common#get_pluginbranch(plugin)
          let params += [{
            \   'name': printf('%s/%s', user_name, pkgsync#common#get_pluginname(plugin)),
            \   'cmd': printf(s:CMDFORMAT_CLONE, empty(branch) ? '' : '--branch ' .. branch, user_name, pkgsync#common#get_pluginname(plugin)),
            \   'cwd': pack_dir,
            \   'arg': has('nvim') ? { 'lines': [] } : tempname(),
            \   'job': v:null,
